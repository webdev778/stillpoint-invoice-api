...
export default class Headline extends React.Component {
    ...
    _handleClick(e) {
        ...
        switch ('photo') {
            ...
            case 'lawyer_headline':
                if (!!networkData.lawyer_headline) {
                    if (networkData.lawyer_headline.length <= 150) {
                        ...
                        fieldValidationErrors.lawyer_headline = constant.INVALID_LAWYER_HEADLINE_LENGTH;
                    }
                } else {
                    this.state.lawerHeadlineValid = true;
                    fieldValidationErrors.lawyer_headline = '';
            ...
            case 'about_lawyer':
                if (!!networkData.about_lawyer) {
                    if (networkData.about_lawyer.length <= 700) {
                        ...
                        fieldValidationErrors.about_lawyer = constant.INVALID_ABOUT_ME_LENGTH;
                    }
                } else {
                    this.state.aboutLawyerValid = true;
                    fieldValidationErrors.about_lawyer = '';
            ...
            case 'linkedin_link':
                if (!!networkData.linkedin_link) {
                    ...
                    if ((link.includes('https://') || link.includes('http://') || link.includes('www.')) && link.includes('linkedin.com')) {
                        this.state.linkedinValid = true;
                        fieldValidationErrors.linkedin_link = '';
                    } else {
                        this.state.linkedinValid = false;
                        fieldValidationErrors.linkedin_link = constant.INVALID_LINKEDIN_LINK;
                    }
                } else {
                    this.state.linkedinValid = true;
                    fieldValidationErrors.linkedin_link = '';
        ...
        if (this.state.photoValid && this.state.lawerHeadlineValid && this.state.aboutLawyerValid && this.state.linkedinValid && this.state.resumeValid && this.state.sampleValid) {
            ...
            utils.apiCall('SET_USER_NETWORK_PROFILE', {'data': data}, function (err, response) {
                if (err) {
    ...
    onImageCrossIconClick() {
        ...
        _that.setState({
            modalPopupObj: {
                ...
                msg: constant['POPUP_MSG']['CONFIRM_MSG'] + constant['POPUP_MSG']['DELETE_PHOTO'],
                noBtnText: 'Cancel',
                yesBtnText: 'OK',
    ...
    render() {
        ...
        return (
            <Role step="headline" role="seeker" profileStatus={this.state.completeStatus}>
                <div className="visible-xs mobile-page-heading">
                    <span className="previous"
                          onClick={() => utils.changeUrl(routesPath['SEEKER_EXEPERIENCE'])}></span> Headline +
                    Additional Info <span className={this.state.completeStatus >= 3 ? 'next' : 'next disabled-element'}
                                          onClick={() => utils.changeUrl(routesPath['SEEKER_JOB_TYPE'])}></span>
                </div>
                    ...
                    <div className="network-card card">
                        ...
                        <div className="upload-photo">
                            <div className="form-group">
                                ...
                                <AvatarCropper onRequestHide={this.handleRequestHide}
                                               cropperOpen={this.state.cropperOpen}
                                               onCrop={this.handleCrop} image={this.state.img} width={200}
                                               height={200}/>
                                }
                            </div>
                            <span className={this.state.deleteDisabled ? 'd-none' : "d-block"} data-toggle="modal"
                                  onClick={this.onImageCrossIconClick}>
                <img src={constant['IMG_PATH'] + 'svg-images/error.svg'}/>
              </span>
                            <div className={this.state.formErrors.photo !== '' ? 'caption global-error' : 'caption'}>
                                upload photo
                                <p>
                                    <span>{this.state.formErrors.photo !== '' ? this.state.formErrors.photo : ''}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="row">
                            <div className="col-sm-6">
                                <div className="row m-0">
                                    <div
                                        className={this.state.formErrors.lawyer_headline !== '' ? 'form-group global-error' : 'form-group'}>
                                        <label htmlFor="lawyer-head" className="control-label">My Headline</label>
                                        <textarea id="lawyer-head" name="lawyer_headline"
                                                  value={this.state.network.lawyer_headline}
                                                  onBlur={this.handleInputOnBlur} onChange={this.handleUserInput}
                                                  className="form-control"
                                                  placeholder="Please create a brief headline for your Legably profile"></textarea>
                                        <p>
                                            <span>{this.state.formErrors.lawyer_headline !== '' ? this.state.formErrors.lawyer_headline : ''}</span>
                                        </p>
                                    </div>
                                </div>
                                <div className="row m-0">
                                    <div
                                        className={this.state.formErrors.linkedin_link !== '' ? 'form-group global-error' : 'form-group'}>
                                        <label htmlFor="linkedin-link" className="control-label">LinkedIn</label>
                                        <input type="text" name="linkedin_link" value={this.state.network.linkedin_link}
                                               onBlur={this.handleInputOnBlur} onChange={this.handleUserInput}
                                               id="linkedin-link" className="form-control" placeholder="LinkedIn"/>
                                        <p>
                                            <span>{this.state.formErrors.linkedin_link !== '' ? this.state.formErrors.linkedin_link : ''}</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="col-sm-6">
                                <div
                                    className={this.state.formErrors.about_lawyer !== '' ? 'form-group global-error' : 'form-group'}>
                                    <label htmlFor="about-lawyer" className="control-label">About Me</label>
                                    <textarea id="about-lawyer" name="about_lawyer"
                                              value={this.state.network.about_lawyer} onBlur={this.handleInputOnBlur}
                                              onChange={this.handleUserInput} className="form-control about-lawyer"
                                              placeholder="Please tell us a bit about you, your background and experience, and the types of opportunities you're seeking"></textarea>
                                    <p>
                                        <span>{this.state.formErrors.about_lawyer !== '' ? this.state.formErrors.about_lawyer : ''}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="row">
                            <div
                                className={this.state.formErrors.resume !== '' ? 'col-sm-6 upload-document global-error' : 'col-sm-6 upload-document'}>
                                <div className="dropzone">
                                    <Dropzone
                                        accept="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                                        onDrop={this.onDropFile} maxSize={this.state.maxSize}
                                        multiple={this.state.multiple}/>
                                    <div
                                        className={(this.state.resumeName != '') ? "m-0 form-group row d-none" : "m-0 form-group row d-block"}>
                    <span className="d-inline-block">
                      <img src={constant['IMG_PATH'] + 'upload-doc.png'} alt="upload-doc"
                      ...
                      <p>Drag and Drop  or Click to Select</p>
                                    ...
                                    <aside className={(this.state.resumeName != '') ? "pt-50 d-block" : "d-none"}>
                                        <ul>
                                            <li>
                                                ...
                                                <span title={this.state.resumeName}
                                                      className="trunc">{this.state.resumeName}</span>
                          ...
                          <i className="fa fa-trash-o" aria-hidden="true"></i>
                                ...
                                <div className="supports pull-right">Supported Format: Pdf, Doc | Size 3 MB</div>
                                <p className="">
                                    <span
                                        className="m-0">{this.state.formErrors.resume !== '' ? this.state.formErrors.resume : ''}</span>
                                </p>
                                <span className="clearfix"></span>
                            </div>

                            <div
                                className={this.state.formErrors.writing_samples !== '' ? 'col-sm-6 upload-document global-error' : 'col-sm-6 upload-document'}>
                                <div className="dropzone">
                                    <Dropzone
                                        accept="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                                        onDrop={this.onDrop} maxSize={this.state.maxSize}/>
                                    <div
                                        className={(this.state.alreadyAccepted.length > 0 || this.state.accepted.length > 0 || this.state.rejected.length > 0) ? "m-0 form-group row d-none" : "m-0 form-group row d-block"}>
                    <span className="d-inline-block">
                      <img src={constant['IMG_PATH'] + 'upload-doc.png'} alt="upload-doc"
                           className="img-responsive pull-right"/>
                    </span>
                      ...
                      <p>Drag and Drop or Click to Select</p>
                                    ...
                                    <aside
                                        className={(this.state.alreadyAccepted.length > 0 || this.state.accepted.length > 0 || this.state.rejected.length > 0) ? "d-block" : "d-none"}>
                                        <div
                                            className={this.state.expand ? "file-wrapper" : "file-wrapper max-height-255"}>
                                            <ul>{this.state.alreadyAccepted.map((f, index) => <li key={index}><i
                                                className="fa fa-file-text-o" aria-hidden="true"></i> <span
                                                className="trunc" title={f.name}>{f.name}</span> <span
                                                onClick={(e) => this.deleteFile('alreadyAccepted', index)}> <i
                                                className="fa fa-trash-o" aria-hidden="true"></i></span></li>)}</ul>
                                            <ul>{this.state.accepted.map((f, index) => <li key={index}><i
                                                className="fa fa-file-text-o" aria-hidden="true"></i> <span
                                                className="trunc" title={f.name}>{f.name}</span> <span
                                                onClick={(e) => this.deleteFile('accepted', index)}> <i
                                                className="fa fa-trash-o" aria-hidden="true"></i></span></li>)}</ul>
                                            <ul>{this.state.rejected.map((f, index) => <li key={index}><i
                                                className="fa fa-file-text-o" aria-hidden="true"></i> <span
                                                className="trunc" title={f.name}>{f.name}</span> <span
                                                onClick={(e) => this.deleteFile('rejected', index)}> <i
                                                className="fa fa-trash-o" aria-hidden="true"></i> </span><p>
                                                <span>{f.error}</span></p></li>)}</ul>
                                        </div>
                                        <p className={(this.state.alreadyAccepted.length + this.state.accepted.length + this.state.rejected.length) > 1 ? "drag-drop pull-left" : "drag-drop"}>Drag
                                            and Drop or Click to Select</p>
                                        <div className="expand-hide-list pull-right">
                                            <a className={this.state.expand && ((this.state.alreadyAccepted.length + this.state.accepted.length + this.state.rejected.length) > 1) ? "expand-list" : "expand-list d-none"}
                                               onClick={() => this.expandHide('expand')}>Show More</a>
                                            <a className={!this.state.expand && ((this.state.alreadyAccepted.length + this.state.accepted.length + this.state.rejected.length) > 1) ? "hide-list" : "hide-list d-none"}
                                               onClick={() => this.expandHide('hide')}>Show Less</a>
                                        </div>
                                    </aside>
                                </div>
                                ...
                                <p>
                                    <span
                                        className="d-none m-0">{this.state.formErrors.writing_samples !== '' ? this.state.formErrors.writing_samples : ''}</span>
                                </p>
                    ...
                    <div className="nxt-prev-btns">
                        <button type="button" onClick={() => utils.changeUrl(routesPath['SEEKER_EXEPERIENCE'])}
                                className="previouse-btn btn pull-left"> Previous
                        </button>
                        <div className={this.state.editProfile == true ? "d-block" : "d-none"}>
                            <button type="button" name="save&Next" className="nxt-btn btn-primary btn pull-right"
                                    onClick={this._handleClick}> Save & Next
                            </button>
                            <button type="button" name="save" className="nxt-btn btn-primary btn pull-right mr-1p"
                                    onClick={this._handleClick}> Save
                            </button>
                            <button type="button" className="nxt-btn btn-default btn pull-right mr-1p"
                                    onClick={() => utils.refreshPage()}> Cancel
                            </button>
                        </div>
                        <button type="submit" name="save&Next" onClick={this._handleClick}
                                className={this.state.editProfile == true ? "d-none nxt-btn btn pull-right" : "d-block nxt-btn btn-primary btn pull-right"}> Next
                        </button>
