...
function _actionsAfterUpdateJobStatus(dataObj, callback) {
    ...
    switch (dataObj['status']) {
        case 101:
            _sendAppliedMail(dataObj, callback);
            break;
        case 103:
            _createNegotiateTerms(dataObj, callback);
            break;
        case -103:
            _updateNegotitateTermStatus(dataObj, callback);
            break;
        case 104:
            _sendMailOnAcceptTerms(dataObj, callback);
            break;
        case -104:
            _updateStripeChargeStatus(dataObj, callback);
            break;
        default:
            callback(null, true);
    }
