...
function contactUs(req, res, callback) {
  ...
  if (!!reqBody.first_name && !!reqBody.last_name && !!reqBody.subject && !!reqBody.email && !!reqBody.message) {
    if (validator.alphaWithDashOnly(reqBody.first_name) && validator.maxLength(reqBody.first_name, 50, true) && validator.alphaWithDashOnly(reqBody.last_name) && validator.maxLength(reqBody.last_name, 50, true) && validator.emailValidation(reqBody.email) && validator.maxLength(reqBody.subject, 50, true) && validator.maxLength(reqBody.message, 1000, true)) {
      let mailObj = {
      ...
      config.mailOptions.html = mailHelper.getMailTmpl('CONTACT_US_FOR_SUPPORT', mailObj);
      ...
      config.transporter.sendMail(config.mailOptions, (error, info) => {
        if(error) {
          ...
          utils.writeErrorLog('universal', 'contactUs', 'Error while sending email to Legably', error, {'email': constant['MAIL_OBJ']['SUPPORT_ID']});
        } else {
          ...
          config.mailOptions.html = mailHelper.getMailTmpl('CONTACT_US_FOR_USER', mailObj);

          // send mail with defined transport object
          config.transporter.sendMail(config.mailOptions, (error, info) => {
            if (error) {
              callback({Code:400, Status:false, Message:constant['OOPS_ERROR']});
              utils.writeErrorLog('universal', 'contactUs', 'Error while sending email to User', error, {'email': mailObj['email']});
            } else {
              callback({Code:200, Status:true, Message:constant['SENT_CONTACTUS_MAIL']});
            }
          });
        }
      });
...
function getAllListsData(req, res, callback) {
  ...
  async.forEachOf(listsObj, (value, key, cb) => {
    ...
    if (err) {
      ...
      utils.writeErrorLog('universal', 'getAllListsData', 'Error while getting dropdown data', err);
    }
    callback(resObj);
  });
}
          ...
          if (pResult.length > 0) {
            ...
            userSchema.findQuery(dbQueryParam, function(uErr, uResult) {
              if (uResult.length > 0) {
                ...
                config.mailOptions.html = mailHelper.getMailTmpl('SEND_MESSAGE', mailObj);
                // send mail with defined transport object
                config.transporter.sendMail(config.mailOptions, (error, info) => {
                  if (error) {
                    utils.callCB(cb, resObj);
                    utils.writeErrorLog('universal', 'sendMsg', 'Error while sending email', error, {'email': uResult[0]['email']});
                  } else {
                ...
                utils.callCB(cb, resObj);
                utils.writeErrorLog('universal', 'sendMsg', 'Error while getting user detail', (uErr || uResult), dbQueryParam['query']);
              }
            });
          } else {
            utils.callCB(cb, resObj);
            utils.writeErrorLog('universal', 'sendMsg', 'Error while getting job detail', (pErr || pResult), dbQueryParam['query']);
          }
        });
      } else {
