...
function webhook(req, res) {
    ...
    switch (reqBody['type']) {
        case constant['STRIPE_EVENT_TYPE']['ACCOUNT_APPLICATION_AUTHORIZED']:
            _updateStripeAccountStatus(reqBody, constant['STRIPE_ACCOUNT_STATUS']['ACTIVATED']);
            break;
        case constant['STRIPE_EVENT_TYPE']['TRANSFER_FUND_PAID']:
            _fundsInStripe(reqBody, constant['PAYMENT_STATUS']['FUND_TRANSFER_SUCCESSFUL']);
            break;
        case constant['STRIPE_EVENT_TYPE']['RELEASE_PAYMENT_PAID']:
            _updateMilestomePaymentStatus(reqBody, constant['DELIVERABLE_STATUS']['PAID']);
            break;
    }
...
function _updateStripeAccountStatus(webhookObj, status) {
    utils.writeInsideFunctionLog('stripeAccounts', '_updateStripeAccountStatus', {
        'stripeAccount': webhookObj['account'],
        'status': status
    });
