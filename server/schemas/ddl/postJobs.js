...
postJobs.statics.findQuery = function(queryObj = {}, callback) {
  this.find(queryObj.query || {}, queryObj.options || {}).skip(queryObj.skip || 0).limit(queryObj.limit || 0).sort(queryObj.sortOption || {}).exec(callback);
};

postJobs.statics.updateQuery = function(id, data,callback) {
  this.findOneAndUpdate({_id: id}, {$set: data}, {new: true, upsert: false}, callback);
};

postJobs.statics.getCount = function(queryObj = {}, callback){
  this.find(queryObj.query || {}).count().exec(callback);
}
