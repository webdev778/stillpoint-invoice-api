                ...
                <div className="common-top-content">
                    {{
                        basic_info_edu: [
                            ...
                            <p key="desc">To get started, please let us know whether you are interested in using Legably
                                to find a job or to post a job by making a selection below. Then, complete the following
                                form to proceed.</p>
                        ],
                        experience: [
                            <h3 key="title">Experience</h3>,
                            <p key="desc">Next, please provide us with some basic information regarding your work
                                experience.</p>
                        ],
                        headline: [
                            <h3 key="title">Headline + Additional Info</h3>,
                            <p key="desc">Next, please create your Legably profile headline and provide a brief bio,
                                your resume, and writing samples or other sample work-product (optional).</p>
                        ],
                        job_type: [
                            <h3 key="title">Job Type</h3>,
                            <p key="desc">Finally, please provide details regarding the type of opportunities you are
                                interested in exploring on Legably.</p>
                        ],
                        get_started: [
                    ...
                    <div className="tabs">
                        <button name="seeker"
                                className={'btn tab-btn-white m-0' + (this.isSeekerProfile() ? ' tab-btn-blue' : '')}
                                onClick={() => this.showPopup('seeker')}>I want to find a job
                        </button>
                        <button name="poster"
                                className={'btn tab-btn-white m-0' + (this.isSeekerProfile() ? '' : ' tab-btn-blue')}
                                onClick={() => this.showPopup('poster')}>I want to post a job
                        </button>
                    </div>
                </div>
                {this.isSeekerProfile() ?
                    <SeekerSteps step={this.props.step} profileStatus={this.props.profileStatus}/> :
                    <PosterSteps step={this.props.step} profileStatus={this.props.profileStatus}/>}
                ...
                <div className="board-inner">
                    <ul className="nav nav-tabs" id="myTab">
                        ...
                        <li className={`${this.props.step === 'basic_info' ? 'active fill' : 'fill'}`}>
                            ...
                            <span
                                onClick={() => utils.changeUrl(routesPath['POSTER_BASIC_INFO'])}>Basic Information</span><span></span>
                        </li>
                        ...
                        <li className={`${this.props.step === 'thank_you' ? 'active' : ''} ${((this.state.stepArr).indexOf(this.props.step) >= 1 || this.props.profileStatus === 1) ? 'fill' : ''} ${(this.props.profileStatus < 1) ? 'disabled-element' : ''}`}>
                            ...
                            <span
                                onClick={() => utils.changeUrl(routesPath['POSTER_THANK_YOU'])}>Thank You!</span><span></span>
                        </li>
                    </ul>
                </div>
            </section>
                ...
                <div className="board-inner">
                    <ul className="nav nav-tabs" id="myTab">
                        ...
                        <li className={`${this.props.step === 'experience' ? 'active' : ''} ${((this.state.stepArr).indexOf(this.props.step) >= 1 || this.props.profileStatus === 4) ? 'fill' : ''} ${(this.props.profileStatus < 1) ? 'disabled-element' : ''}`}>
                            ...
                            <span
                                onClick={() => utils.changeUrl(routesPath['SEEKER_EXEPERIENCE'])}>Experience</span><span></span>
                        </li>
                        ...
                        <li className={`${this.props.step === 'job_type' ? 'active' : ''} ${((this.state.stepArr).indexOf(this.props.step) >= 3 || this.props.profileStatus === 4) ? 'fill' : ''} ${(this.props.profileStatus < 3) ? 'disabled-element' : ''}`}>
                            ...
                            <span
                                onClick={() => utils.changeUrl(routesPath['SEEKER_JOB_TYPE'])}>Job Type</span><span></span>
                        </li>
