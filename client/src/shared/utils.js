...
const loader = (action) => {
  ...
  (action === 'start') ? legablyLoader.removeClass('hide') : legablyLoader.addClass('hide');
}

const flashMsg = (action, msg, type = 'error') => {
  let legablyFlashMsg = $('#legably_flash_msg');
  if (action === 'show') {
    legablyFlashMsg.removeClass('hide').children('#msg').text(msg);
...
const apiCall = (key, options = {}, callback) => {
  ...
  axios(axiosObj).then((response) => { // success
    if (response.data.Message === constant.AUTH_FAIL || response.data.message === constant.AUTH_FAIL) {
...
const isResSuccess = (res) => (res['data']['code'] === constant['HTTP_STATUS_CODES']['SUCCESS'] && res['data']['status'] === true);

const isResConflict = (res) => (res['data']['code'] === constant['HTTP_STATUS_CODES']['CONFLICT']);

const isResPreconditionReq = (res) => (res['data']['code'] === constant['HTTP_STATUS_CODES']['PRECONDITION_REQUIRED']);

const isResLocked = (res) => (res['data']['code'] === constant['HTTP_STATUS_CODES']['LOCKED']);

const isResBarIdValid = (res) => (res['data']['code'] === constant['HTTP_STATUS_CODES']['INVALID_BAR_ID']);
...
const logger = (type, msg, value = null) => {
  ...
  switch(type) {
    case 'error': configObj['SHOW_CLIENT_ERROR'] && console.error(msg, (value === null ? '' : value));
    break;
    case 'warn': configObj['SHOW_CLIENT_WARN'] && console.warn(msg, (value === null ? '' : value));
    break;
    case 'log': configObj['SHOW_CLIENT_LOG'] && console.log(msg, (value === null ? '' : value));
    break;
  }
}
...
const setAllListsData = (showLoader) => {
  if (legablyStorage.get(constant['API_URLS']['GET_ALL_LISTS']['name']).length === 0) {
    apiCall('GET_ALL_LISTS', {'showLoader': showLoader}, function(err, response) {
      if (err) {
...
const getListDataRelatedToIds = (key, idsArr = []) => {
  let dataArr = legablyStorage.get(constant['API_URLS']['GET_ALL_LISTS']['name']);
...
const moveToLastUpdatedEditProfilePage = (isMovementForSeeker) => {
  if (getUserRole() === 'admin') {
    ...
    apiCall('GET_USER_PROFILE', { 'params': ['job_seeker_info', 'job_posters_info'] }, function(err, response) {
      if (err) {
