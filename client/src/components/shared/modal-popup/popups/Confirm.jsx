...
export default class Confirm extends React.Component {
    ...
    getMessage() {
        ...
        return (
            msgArr.length ?
                msgArr.map((msg, index) => <p className="modal-para" key={index}
                                              dangerouslySetInnerHTML={{__html: msg}}></p>)
                :
                null
        )
    }
        ...
        return (
            <div className="alert-modal">
                ...
                <div className="modal-footer">
                    <button
                        className={(this.popupObj.noBtnText === 'Cancel') ? 'btn-negative btn pull-left' : 'btn-no btn pull-left'}
                        onClick={() => this.onNoBtnClick()}>{this.popupObj.noBtnText || 'No'}</button>
                    <button className="btn-primary btn pull-right"
                            onClick={() => this.onYesBtnClick()}>{this.popupObj.yesBtnText || 'Yes'}</button>
                </div>
            </div>
        )
    }
