...
saved_jobs = mongoose.model('saved_jobs'),
...
function _sendMailOnPostJob(userDataObj, jobDataObj) {
  ...
  mailHelper.sendMailInBackground(userDataObj['email'], 'Job Posted', 'JOB_POSTED', mailObj);
}

function _postJobSaveData(req, res, userData, callback) {
  utils.writeInsideFunctionLog('postJobs', '_postJobSaveData', {'_id': (req['body']['_id'] || null)});
  ...
  if (!req.body._id) {
    ...
    post_jobs.saveData(req['body'], function(err, resp) {
      if (err) {
        ...
        utils.writeErrorLog('postJobs', '_postJobSaveData', 'Error while creating entry in post job', err, req['body']);
      } else {
        if (req.body.status === constant['STATUS']['ACTIVE']) {
          _sendMailOnPostJob(userData, req['body']);
        }
        callback({Code: 200, Status: true, Message: constant['SUCCESS_POST_JOB']});
      }
    });
  } else {
    let id = req.body._id;
    ...
    post_jobs.updateQuery(id, req.body, function(err, resp) {
      if (err) {
        callback({Code: 400, Status: false, Message: constant['OOPS_ERROR']});
        utils.writeErrorLog('postJobs', '_postJobSaveData', 'Error while updating job detail', err, {'_id': id});
      } else {
        let dbQueryParams = {
          'query': {'jobId': id, 'status': constant['N_TERMS_STATUS']['NOT_SENT']}
        }
        delete req['body']['status'];
        nTermsSchema.updateQuery(dbQueryParams, req['body'], function(nErr, nRes) {
          if (nErr) {
            callback({Code: 400, Status: false, Message: constant['OOPS_ERROR']});
            utils.writeErrorLog('postJobs', '_postJobSaveData', 'Error while updating negotiate terms detail', nErr, dbQueryParams['query']);
          } else {
            callback({Code: 200, Status: true, Message: constant['SUCCESS_POST_JOB']});
          }
        })
...
function _savePostJob(req, res, userData, callback) {
  ...
  employment_types.findJobType(req.body.hoursType, function(error, response){
    if (error) {
      callback({Code:400, Status:false, Message:constant['OOPS_ERROR']});
      utils.writeErrorLog('postJobs', '_savePostJob', 'Error while getting employment types detail', error, {'_id': req.body.hoursType});
    } else {
      if (response) {
        if (response.name == "Part-time" || response.name == "Full-time") {
          ...
          if (Number(req.body.subTotal).toFixed(2) == subtotalAmt) {
            ...
            if (Number(req.body.total).toFixed(2) == Number(total).toFixed(2)) {
              if (req.body.paymentDetails.length>0 && !(req.body.jobType == '1099' && req.body.paymentType == 'Hourly Rate/Fixed Fee')) {
                ...
                for (var i=0; i<req.body.paymentDetails.length; i++) {
                  if (req.body.paymentDetails[i].rate == 0 && !req.body.paymentDetails[i].delivery && !req.body.paymentDetails[i].dueDate) {
                    ...
                    if (req.body.paymentDetails[i].dueDate && !req.body._id) {
                      if (moment(req.body.paymentDetails[i].dueDate).isSameOrAfter(utils.getCurrentEstDate())) {
                        count++;
                      } else {
                        callback({Code:400, Status:false, Message:constant['INVALID_DUE_DATE']});
                      }
                    } else {
                      count++;
                    }
              ...
              callback({Code:400, Status:false, Message:constant['TOTAL_AMT_ERROR']});
            }
          } else {
            callback({Code:400, Status:false, Message:constant['SUBTOTAL_ERROR']});
          }
        } else {
          callback({Code:404, Status:false, Message:constant['INVALID_HOUR_TYPE_ID']});
        }
      } else {
        callback({Code:404, Status:false, Message:constant['INVALID_HOUR_TYPE_ID']});
      }
...
function _getStepData(stepDataObj, callback) {
  utils.writeInsideFunctionLog('postJobs', '_getStepData', {'userId': stepDataObj['userId'], 'userRole': stepDataObj['userRole'], 'jobId': stepDataObj['jobId'], 'step': stepDataObj['step']});

  if (stepDataObj['userRole'] === constant['ROLE']['SEEKER']) {
    switch (stepDataObj['step']) {
      case -101:
      case 101: _getAppliedDate(stepDataObj, callback);
      break;
      case -102:
      case -104: _getDeclinedBy(stepDataObj, callback);
      break;
      case -103: _getDeclinedByWithTermsStatus(stepDataObj, callback);
      break;
      case 103: negotiateTermsModel.get(stepDataObj, callback);
      break;
      case 104: _getWNineAndSetPreferenceDetails(stepDataObj, callback);
      break;
      case 105: _getInProgressDetails(stepDataObj, callback);
      break;
      default: callback(null, []);
    }
  } else {
    let absStep = (stepDataObj['step'] < 0) ? constant['JOB_STEPS']['APPLIED'] : stepDataObj['step'];
    switch (absStep) {
      case 101: _getApplicantListWithDeclinedCandidates(stepDataObj, callback);
      break;
      case 102: _getApplicantList(stepDataObj, callback);
      break;
      case 103: negotiateTermsModel.get(stepDataObj, callback);
      break;
      case 104: _getStartPendingDetails(stepDataObj, callback);
      break;
      case 105: _getInProgressDetails(stepDataObj, callback);
      break;
      default: callback(null, []);
...
function _getAppliedDate(stepDataObj, callback) {
  ...
  job_status.findQuery(queryObj, function(err, res){
    if (!!res && res.length) {
      res[0]['_doc']['created_at'] = utils.convertUtcToEst(res[0]['created_at']).format();
      callback(null, res);
    } else {
      callback(constant['OOPS_ERROR']);
      utils.writeErrorLog('postJobs', '_getAppliedDate', 'Error while getting job status detail', (err || res), queryObj['query']);
    }
  });
}
...
function _getDeclinedBy(stepDataObj, callback) {
  ...
  job_status.findQuery(dbQueryParams, function(err, res){
    if (!!res && res.length) {
      ...
      callback(err);
      utils.writeErrorLog('postJobs', '_getDeclinedBy', 'Error while getting job status detail', (err || res), dbQueryParams['query']);
    }
  });
}
    ...
    if (error) {
      ...
      nTermsSchema.findQuery(dbQueryParams, function(err, res) {
        if (!!res && res.length) {
          ...
          callback(err);
          utils.writeErrorLog('postJobs', '_getDeclinedByWithTermsStatus', 'Error while getting negotiate terms detail', (err || res), dbQueryParams['query']);
        }
      });
    }
  });
...
function _getWNineAndSetPreferenceDetails(stepDataObj, cb) {
  ...
  wNineInfoModel.get(stepDataObj, function(err, res) {
    if (err) {
      ...
      res[0]['stripe_account_status'] = constant['STRIPE_ACCOUNT_STATUS']['NOT_CREATED'];
      stripeAccountsSchema.findAccountDetails(stepDataObj, function(sErr, sRes) {
        if (sErr) {
          callback(constant['OOPS_ERROR']);
          utils.writeErrorLog('postJobs', '_getWNineAndSetPreferenceDetails', 'Error while getting stripe account detail', (sErr || sRes), stepDataObj['userId']);
        } else {
          let accountDetails = sRes[0];
          accountDetails && (res[0]['stripe_account_status'] = accountDetails['status']);
          cb(null, res);
        }
      });
    }
...
function _maintainOrder(arr, arrOfObj) {
  var temp = [];
  for (var i = 0, len = arrOfObj.length; i < len; i++) {
    var index = arr.indexOf(arrOfObj[i]._id.toString());
...
function _getApplicantListWithDeclinedCandidates(stepDataObj, callback) {
  utils.writeInsideFunctionLog('postJobs', '_getApplicantListWithDeclinedCandidates');
  ...
  post_jobs.getApplicantsWithDeclined(dbQueryParams, function(pErr, pRes){
    if (!!pRes && pRes.length) {
      ...
      users.findQuery(queryObj, function(uErr, uRes) {
        if (uErr) {
          callback(constant['OOPS_ERROR']);
          utils.writeErrorLog('postJobs', '_getApplicantListWithDeclinedCandidates', 'Error while getting user detail', uErr, queryObj['query']);
        } else {
          ...
          users.findQuery(queryObj, function(dErr, dRes) {
            if (dErr) {
              callback(constant['OOPS_ERROR']);
              utils.writeErrorLog('postJobs', '_getApplicantListWithDeclinedCandidates', 'Error while getting declined user detail', dErr, queryObj['query']);
            } else {
              ...
              for (let user of dRes) {
                let index = declinedApplicantsIdArr.indexOf(user['_id'].toString());
                if (index > -1 ) {
                  let declinedApplicantObj = pResult['declined_applicants'][index];
                  user['_doc']['declined_by'] = declinedApplicantObj['declined_by'];
      ...
      callback(constant['OOPS_ERROR']);
      utils.writeErrorLog('postJobs', '_getApplicantListWithDeclinedCandidates', 'Error while getting job and user detail', (pErr || pRes), dbQueryParams);
    }
  });
}
  ...
  post_jobs.getApplicants(dbQueryParams, function(pErr, pRes) {
    if (!!pRes && pRes.length) {
      ...
      users.findQuery(queryObj, function(uErr, uRes) {
        if (uErr) {
          callback(constant['OOPS_ERROR']);
          utils.writeErrorLog('postJobs', '_getApplicantList', 'Error while getting user detail', uErr, queryObj['query']);
        } else {
          callback(null, uRes);
        }
      });
      ...
      callback(constant['OOPS_ERROR']);
      utils.writeErrorLog('postJobs', '_getApplicantList', 'Error while getting job and user detail', (pErr || pRes), dbQueryParams);
    }
  });
}
...
function _getStartPendingDetails(stepDataObj, callback) {
  ...
  stripeAccountsSchema.findAccountDetails(stepDataObj, function(sErr, sResult) {
    if (sErr) {
      callback(constant['OOPS_ERROR']);
      utils.writeErrorLog('postJobs', '_getStartPendingDetails', 'Error while getting user stripe account detail', sErr, stepDataObj);
    } else {
      let accountDetails = sResult[0],
      stripe_account_status = constant['STRIPE_ACCOUNT_STATUS']['NOT_CREATED'],
      transfer_funds_status = constant['PAYMENT_STATUS']['FUND_TRANSFER_REQUEST_NOT_SENT'];
      if (accountDetails) {
        stripe_account_status = accountDetails['status'];
        accountDetails['tranfer_funds'].length > 0 && (transfer_funds_status = accountDetails['tranfer_funds'][0]);
      }
      let queryObj = {
        'query': {
      ...
      nTermsSchema.findQuery(queryObj, function(nErr, nRes) {
        if (nErr) {
          callback(constant['OOPS_ERROR']);
          utils.writeErrorLog('postJobs', '_getStartPendingDetails', 'Error while getting negotiate terms detail', nErr, queryObj);
        } else {
          let dataObj = {'job_id': stepDataObj['jobId'], 'status':  stepDataObj['step']};
          job_status.findBarIdStatus(dataObj, function(jErr, jRes) {
            if (!!jRes && jRes.length) {
              let data = jRes[0];
              callback(null, [{'seekerId': data['user_id'], 'freeze_activity': data['freeze_activity'][0], 'stripe_account_status': stripe_account_status, 'transfer_funds_status': transfer_funds_status, 'amount': nRes[0]['total']}]);
            } else {
              callback(constant['OOPS_ERROR']);
              utils.writeErrorLog('postJobs', '_getStartPendingDetails', 'Error while getting user, job and stripe account detail', (jErr || jRes), dataObj);
            }
          });
        }
      });
...
function _getInProgressDetails(stepDataObj, callback) {
  ...
  nTermsSchema.getNonPaidDeliverables(data, function(err, res) {
    if (err) {
      callback(constant['OOPS_ERROR']);
      utils.writeErrorLog('postJobs', '_getInProgressDetails', 'Error while getting non paid deliverable detail', err, data);
    } else {
      let milestone_id = res.length ? res[0]['paymentDetails']['_id'] : null;
      nTermsSchema.getDeliverables(data, function(cErr, cData) {
        if (cErr) {
          callback(cErr);
          utils.writeErrorLog('postJobs', '_getInProgressDetails', 'Error while getting deliverables count', cErr, data);
        } else {
          data['count'] = false;
          nTermsSchema.getDeliverables(data, function(nErr, nRes) {
            if (nErr) {
              callback(nErr);
              utils.writeErrorLog('postJobs', '_getInProgressDetails', 'Error while getting deliverables detail', nErr, data);
            } else {
              let result = { 'count': cData.length, 'action_milestone_id': milestone_id, 'data': nRes};
              callback(null, result);
            }
          });
        }
  ...
  if (!!req.headers.token) {
    helper.checkUserLoggedIn(req.headers.token , function(err, result){
      if (err) {
        ...
        if (!!req.params.jobId) {
          post_jobs.getJobData({_id : req.params.jobId}, function(pErr, pResult) {
            if (pErr) {
              callback({Code:400, Status:false, Message:constant['OOPS_ERROR']});
              utils.writeErrorLog('postJobs', 'getPostjobData', 'Error while getting job detail', pErr, {'_id' : req.params.jobId});
            }else{
              if(pResult !== null){
                callback({Code:200, Status:true, Message:constant['REQUEST_OK'], Data:pResult});
              }else{
          ...
          if (cErr) {
            ...
            if (cResult > 0) {
              post_jobs.getAllJobs(dbQueryParams, function(pErr, pResult) {
                if (pErr) {
                  callback({Code:400, Status:false, Message:constant['OOPS_ERROR']});
                  utils.writeErrorLog('postJobs', 'getAllPostJobs', 'Error while getting all job details', pErr, dbQueryParams);
                } else {
                  if (pResult !== null) {
                    var obj = {"count": cResult,"data": pResult, "userData": userObj};
    ...
    helper.checkUserLoggedIn(req.headers.token , function(err, result){
      if (err) {
        ...
        post_jobs.getPostjobData(dbQueryParams, function(pErr, pResult){
          if (pErr) {
            utils.callCB(cb, resObj);
            utils.writeErrorLog('postJobs', 'getPostJobDetails', 'Error while getting job details', pErr, dbQueryParams);
          } else {
            if (pResult !== null) {
              var userRole = req.params.userRole;
              if (pResult[0]['current_highest_job_step'] && userRole) {
                var step = pResult[0]['current_highest_job_step'];
                if (userRole === 'seeker') {
                ...
                _getStepData(stepDataObj, function(error, stepData, declinedApplicants) {
                  if (error) {
                    ...
                    utils.callCB(cb, resObj);
                    utils.writeErrorLog('postJobs', 'getPostJobDetails', 'Error from _getStepData', error);
                  } else {
                    resObj = Object.assign({}, utils.getSuccessResObj());
                    pResult[0]['step_data'] = stepData;
                    declinedApplicants && (pResult[0]['declined_candidates'] = declinedApplicants);
                    resObj['data'] = {
                      'job_detail': pResult[0]
                    }
                    if (userRole === 'seeker' && (step === 100 || step === 102 || step === 103 || step === 104)) {
    ...
    helper.checkUserLoggedIn(req.headers.token , function(err, result){
      if(err){
        ...
        post_jobs.getCount(dbQueryParams, function(cErr, cResult){
          if (cErr) {
            utils.callCB(cb, resObj);
            utils.writeErrorLog('postJobs', 'getPostJobByUserId', 'Error while getting job count', cErr, dbQueryParams['query']);
          }else{
            if (cResult > 0) {
              post_jobs.getJobsByUserId(dbQueryParams, function(pErr, pResult){
                if (pErr) {
                  utils.writeErrorLog('postJobs', 'getPostJobByUserId', 'Error while getting job and job status detail', pErr, dbQueryParams['query']);
                } else {
                  if (pResult !== null) {
                    resObj = Object.assign({}, utils.getSuccessResObj());
                    resObj['data'] = {
    ...
    if (err) {
      ...
      _getStepData(stepDataObj, function(error, stepData, declinedApplicants){
        if (error) {
          resObj['message'] = constant['OOPS_ERROR'];
          utils.callCB(cb, resObj);
          utils.writeErrorLog('postJobs', 'getStepData', 'Error from _getStepData', error);
        } else {
          resObj = Object.assign({}, utils.getSuccessResObj());
          resObj['data'] = {
            'step_data': stepData
          }
          declinedApplicants && (resObj['data']['declined_candidates'] = declinedApplicants);
          utils.callCB(cb, resObj);
        }
      });
    }
...
function updateStatus(req, res, cb) {
  ...
  helper.checkUserLoggedIn(req['headers']['token'], function(err, result) {
    if (err) {
      ...
      post_jobs.updateQuery(req['body']['job_id'], dbQueryParams, function(uErr, uRes) {
        if (uErr) {
          utils.writeErrorLog('postJobs', 'updateStatus', 'Error while updating job detail', uErr, {_id : req['body']['job_id']});
        } else {
          resObj = Object.assign({}, utils.getSuccessResObj());
          resObj['data'] = {
           'status': uRes
