...
export default class Profile extends React.Component {
  ...
  render() {
    ...
    return (
      <Dashboard>
        <section className={this.isOtherProfile() ? 'view-profile-wrapper sidebar-margin' : 'view-profile-wrapper ml-auto mr-auto'} ref="profileRef">
          <div className="section-head">
            ...
            </ol>
            {
              this.isOtherProfile() ?
                null
              :
                <button type="button" className="btn edit-profile-btn" onClick={() => this.moveToEditProfilePage()}>
                  <a>
                    ...
                    :
                      <ul className="nav nav-justified">
                        <li className={attorneyProfileTabClass}>
                          <a className="nav-link" id="attorneyPorfile" data-toggle="tab" href="#panel1" role="tab" onClick={(e) => this.changeTab(e)}>Attorney Profile</a>
                        </li>
                        <li className={firmProfileTabClass}>
                          <a className="nav-link" id="firmPorfile" data-toggle="tab" href="#panel2" role="tab" onClick={(e) => this.changeTab(e)}>Firm Profile</a>
                        </li>
                      </ul>
                  }
                     ...
                     <div className={attorneyProfileMainTabClass} id="panel1" role="tabpanel">
                      ...
                      <div className="basic-info-preview">
                          <figure className="pull-left">
                            <img src={this.state.seekerProfileImage} onError={this.profileImgError} />
                          </figure>
                           ...
                           <div className="basic-info-content pull-left">
                            ...
                            <address>
                              <i className="fa fa-map-pin" aria-hidden="true"></i>{seekerAddress}</address>
                            <address>
                              <i className="fa fa-phone" aria-hidden="true"></i>{seekerData.basic_profile.basic_info.phone_number}
                            </address>
                            <Link to={seekerData.network.linkedin_link ? (!seekerData.network.linkedin_link.includes('http') && '//' + seekerData.network.linkedin_link) : null} target="_blank">
                              <i className="fa fa-linkedin" aria-hidden="true"></i>{seekerData.network.linkedin_link ? seekerData.network.linkedin_link:''}
                            </Link>
                          </div>
                          ...
                          <div className="education-preview align-items-center">
...

                            {
                             ...
                             this.state.seekerEducations.map((seekerEducation,index) => {
                                ...
                                return <div key={index} className={ (index> 0 ? 'mt-30' : '') }>
                        ...
                        {/*licensure-preview ends here*/}
                              ...
                              <div className="pull-left left-content">
...

                                      {
                                          ...
                                          this.state.seekerPracticeAreas.map((seekerPracticeArea,index) => {
                                              ...
                                              return <span key={index} className="badge">{seekerPracticeArea}</span>
...

                                      {
                                          ...
                                          this.state.seekerSkills.map((seekerSkill,index) => {
                                              ...
                                              return <span key={index} className="badge">{seekerSkill}</span>
                                          ...
                                          return <div key={index} className={ (index> 0 ? 'mt-30' : '') }>
                                                    {
                                                        ...
                                                        seekerExperience.employment_types.map((employmentType,employmentTypeIndex) => {
                                                            ...
                                                            return <span key={employmentTypeIndex} className="badge">{employmentType}</span>
                              ...
                              <div className="pull-left left-content">
                                <div>
                                  ...
                                  { this.state.seekerJobProfile.willing_to_work_location_id.map(( workLocation,workLocationIndex) => {
                                      return <span key={workLocationIndex} className="badge">{workLocation}</span>
                                    })
                                  }
                                ...
                                <div>
                                  ...
                                  { this.state.seekerJobProfile.desired_job_type.map((desiredJobType,desiredJobTypeIndex) => {
                                      return desiredJobType.selected === 'Y' ? (<h6 key={desiredJobTypeIndex}>{desiredJobType.employment_type} ({desiredJobType.employment_type.toLowerCase() === 'permanent' ? 'Salaried' : 'Hourly Rate'}) {desiredJobType.min_amount == desiredJobType.max_amount ? `$${desiredJobType.max_amount}` : `$${desiredJobType.min_amount} - $${desiredJobType.max_amount}`}</h6>) : null
                                    })
                                  }
                                </div>
                              </div>
                          ...
                          <div className="mt-30 text-right">
                            ...
                            {writingSamples.length > 0 ?
                              <button type="button" className="btn btn-primary" onClick={(e) => this.viewWritingSamples()}>
                    ...
                    <div className={firmProfileMainTabClass} id="panel2" role="tabpanel">
                      ...
                      <div className="poster-info-preview">
                        <div className="poster-info-content">
                          <h3>{this.state.firstName} {this.state.lastName}</h3>
                          <address>
                            <i className="fa fa-map-pin" aria-hidden="true"></i>{posterAddress}
                          </address>
                          <address>
                            <i className="fa fa-phone" aria-hidden="true"></i>{posterData.basic_profile.basic_info.phone_number}
                          </address>
                          <address>
                            <i className="fa fa-globe" aria-hidden="true"></i>{posterData.basic_profile.website_url}
                          </address>
                        </div>
                        <span className="clearfix"></span>
                      </div>
                      ...
                      <div className="poster-conclusion-preview">
                        <div className="pull-left left-content pl-0">
                          <h4>{posterData.basic_profile.firm_name}</h4>
                            ...
                            <div>
                              <h6 className="mr-15 d-inline-block">Practice Location(s)</h6>
                              { this.state.posterPracticeLocations.map(( posterPracticeLocation,posterPracticeLocationIndex) => {
                                  return <span key={posterPracticeLocationIndex} className="badge">{posterPracticeLocation}</span>
                                })
                              }
                             </div>
                              ...
                              <div>
                                ...
                                { this.state.posterPracticeAreas.map(( posterPracticeArea,posterPracticeAreaIndex) => {
                                    return <span key={posterPracticeAreaIndex} className="badge">{posterPracticeArea}</span>
                                  })
                                }
                              </div>
                              <div>
                                <h6 className="mr-15 d-inline-block">Interested in Hiring (Now or in the future)</h6>
                                { this.state.posterInterestedIns.map(( posterInterestedIn,posterInterestedInIndex) => {
                                    return <span key={posterInterestedInIndex} className="badge">{posterInterestedIn}</span>
                                  })
                                }
                              </div>
                            </div>
