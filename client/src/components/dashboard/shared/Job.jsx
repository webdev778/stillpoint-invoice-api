            ...
            if (this.state.jobStatus < 0) {
                ...
                switch (this.state.jobStatus) {
                    ...
                    case constant['JOB_STEPS']['INTERVIEWING']:
                        ...
                        break;
                    case constant['JOB_STEPS']['N_TERMS']:
                        appliedJobStatusText = (job.nTermStatus === constant['N_TERMS_STATUS']['SENT']) ? 'Negotiating Terms' : 'Interviewing';
                        break;
                    case constant['JOB_STEPS']['S_PENDING']:
        ...
        return (
            <div className="card-content">
                ...
                <div>
                    ...
                    <div className="hourly-estimate col-xs-3 col-sm-2 p-0">
                        ${job.rate}
                        <div>{job.rateType ? (job.rateType).toLowerCase() : job.rateType} (Est.)</div>
                    </div>
                ...
                <i className="fa fa-bookmark" aria-hidden="true"></i>
                  {practiceAreas.slice(0, 2).join(", ")}</span>
                            {practiceAreas.length > 2 ?
                                <span className="d-inline-block v-bottom"> +{practiceAreas.length - 2}</span> : ''}
                        </div>
                        : ''
                    }
            ...
            <span className="d-inline-block truncate-80">
              ...
              Est. Time: {job.hours} hrs., {job.duration} {job.durationPeriod}
                ...
                <p className="m-0">
                    <Truncate lines={3}
                              ellipsis={<span>... <Link className="more" to={jobDetailLinkToObj}>more</Link></span>}>
                        {job.jobDescription}
                    </Truncate>
                </p>
                <div className="card-content-footer row">
                    <div className="col-sm-4">
                        <div
                            className="pb-5">Posted:<span>{(this.state.posterJobStatus && this.state.postedAt) ? utils.convertUtcToEst(this.state.postedAt).format(constant['JOB_DATE_FORMAT']) : constant['NO_DATA_SYMBOL']}</span>
                        </div>
                        <div>Desired Start
                            Date:<span>{job.estimatedStartDate ? moment(job.estimatedStartDate).format(constant['JOB_DATE_FORMAT']) : 'ASAP'}</span>
                        </div>
                    </div>
            ...
            */}
                ...
                <img className="mr-10" src={constant['IMG_PATH'] + 'svg-images/user-icon.svg'} alt="user-icon-fill"/>
                        ...
                        {job.fromRoute === 'POSTED_JOBS' ?
                            <button type="button" className={jobBtnClass}
                                    disabled={this.state.posterJobStatus === constant['STATUS']['ACTIVE']}
                                    onClick={this.updatePostedJobStatus.bind(this, job._id, constant['STATUS']['ACTIVE'])}>
                                {postedJobStatusText}
                            </button>
                            :
                            <button type="button" className={jobBtnClass}
                                    disabled={this.state.jobStatus !== constant['JOB_STEPS']['APPLY']}
                                    onClick={this.updateJobStatus.bind(this, job._id, constant['JOB_STEPS']['APPLIED'])}>
