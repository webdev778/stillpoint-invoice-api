...
export default class InProgress extends React.Component {
                                    ...
                                    <div key={index} className="custom-tr">
                                        ...
                                        <div className="custom-td">
                                            {(this.state.jobType == '1099' && this.state.paymentType == 'Hourly Rate/Fixed Fee') ?
                                                ...
                                                :
                                                <span className="delivery-break-word"
                                                      title={item.paymentDetails.delivery}>{item.paymentDetails.delivery || noDataSymbol}</span>
                                            }
                                        </div>
                                        <div className="custom-td">
                                            {(this.state.jobType == '1099' && this.state.paymentType == 'Hourly Rate/Fixed Fee') ?
                                        ...
                                        <div className="custom-td">
                                            {(this.state.jobType == '1099' && this.state.paymentType == 'Hourly Rate/Fixed Fee') ?
                                                <span
                                                    title={item.rate}>${item.rate}{item.rateType == utils.ENUM.RATE_TYPE.HOURLY && '/Hour'}</span>
                                                :
                                                <span
                                                    title={item.paymentDetails.rate}>${item.paymentDetails.rate}</span>
                                            }
                                        </div>
                                        <div className="custom-td">
                                            {
                        ...
                        <div className="dropdown">
                            ...
                            <ul className="dropdown-menu">
                                <li><a href="javascript:void(0)"
                                       className={this.isActive('DUE_DATE', 'ASC') ? 'active' : ''}
                                       onClick={(e) => this.onSortIconClick(e, SORT['BY']['DUE_DATE'], SORT['ORDER']['ASC'])}>Due
                                    Date (Ascending)</a></li>
                                <li role="separator" className="divider"></li>
                                <li><a href="javascript:void(0)"
                                       className={this.isActive('DUE_DATE', 'DESC') ? 'active' : ''}
                                       onClick={(e) => this.onSortIconClick(e, SORT['BY']['DUE_DATE'], SORT['ORDER']['DESC'])}>Due
                                    Date (Descending)</a></li>
                                <li role="separator" className="divider"></li>
                                <li><a href="javascript:void(0)"
                                       className={this.isActive('MILESTONE', 'ASC') ? 'active' : ''}
                                       onClick={(e) => this.onSortIconClick(e, SORT['BY']['MILESTONE'], SORT['ORDER']['ASC'])}>Milestone
                                    (Ascending)</a></li>
                                <li role="separator" className="divider"></li>
                                <li><a href="javascript:void(0)"
                                       className={this.isActive('MILESTONE', 'DESC') ? 'active' : ''}
                                       onClick={(e) => this.onSortIconClick(e, SORT['BY']['MILESTONE'], SORT['ORDER']['DESC'])}>Milestone
                                    (Descending)</a></li>
                                <li role="separator" className="divider"></li>
                                <li><a href="javascript:void(0)"
                                       className={this.isActive('STATUS', 'ASC') ? 'active' : ''}
                                       onClick={(e) => this.onSortIconClick(e, SORT['BY']['STATUS'], SORT['ORDER']['ASC'])}>Status
                                    (A-Z)</a></li>
                                <li role="separator" className="divider"></li>
                                <li><a href="javascript:void(0)"
                                       className={this.isActive('STATUS', 'DESC') ? 'active' : ''}
                                       onClick={(e) => this.onSortIconClick(e, SORT['BY']['STATUS'], SORT['ORDER']['DESC'])}>Status
                                    (Z-A)</a></li>
                            </ul>
                        </div>
                    </h5>
                    <span className="clearfix"></span>
                    {
                        stepRelatedData.map((item, index) => (
                            <div key={index} className="card-view p-0 mb-20">
                                <div className="job-title" data-toggle="collapse"
                                     data-target={'#milestone_' + item.paymentDetails.milestone} aria-expanded="false">
                                    Milestone {item.paymentDetails.milestone}
                                </div>
                                <div className="job-details collapse p-15"
                                     id={'milestone_' + item.paymentDetails.milestone}>
                                    <h6>Deliverable</h6>
                ...
                <div className="custom-tr">
                    <div className="custom-th">
                        <span className="cursor-pointer relative"
                              onClick={(e) => this.onSortIconClick(e, SORT['BY']['MILESTONE'])}>Milestone <i
                            className={this.getSortClass(SORT['BY']['MILESTONE'])}></i></span>
                    </div>
                    <div className="custom-th">Deliverable</div>
                    <div className="custom-th">
                        <span className="cursor-pointer relative"
                              onClick={(e) => this.onSortIconClick(e, SORT['BY']['DUE_DATE'])}>Due Date <i
                            className={this.getSortClass(SORT['BY']['DUE_DATE'])}></i></span>
                    </div>
                    ...
                    <div className="custom-th">
                        {isCompletedDeliverablesTab ?
                            ...
                            :
                            <span className="cursor-pointer relative"
                                  onClick={(e) => this.onSortIconClick(e, SORT['BY']['STATUS'])}>Status <i
                                className={this.getSortClass(SORT['BY']['STATUS'])}></i></span>
                        }
                    </div>
                    {isCompletedDeliverablesTab ?
                        null
    ...
    getDeliverablesStatus(type, paymentDetailObj) {
        ...
        switch (status) {
            case constant['DELIVERABLE_STATUS']['SUBMITTED']:
                statusText = 'Submitted';
                break;
            case constant['DELIVERABLE_STATUS']['APPROVED']:
                statusText = 'Approved';
                break;
            case constant['DELIVERABLE_STATUS']['RELEASED']:
                statusText = 'Payment Released';
                break;
            case constant['DELIVERABLE_STATUS']['PAID']:
                statusText = 'Paid';
                break;
        ...
        return (
            <p className={className}>{statusText} {(status === constant['DELIVERABLE_STATUS']['APPROVED'] && !this.isSeeker()) ?
                <span className="pay-now-link"
                      onClick={() => this.payNow(paymentDetailObj)}>Pay Now</span> : null}</p>);
    }
            ...
            <div className="custom-td">
                {
                    this.state.actionMilestoneId === data['paymentDetails']['_id'] ?
                        <input type="image" disabled={disableBtn} className={imgButtonClass} data-toggle="tooltip"
                               data-placement="bottom" title={imgTitle} alt={imgAlt} src={imgSrc} width="40" height="40"
                               onClick={this.onSubmitBtnClick.bind(this, role, data)}/>
                        :
                        null
                }
                <input type="image" className="img-responsive d-inline" data-toggle="tooltip" data-placement="bottom"
                       title="Send Message" src="/images/message-icon.png" alt="message-icon" width="40" height="40"
                       onClick={this.onSendMsgBtnClick.bind(this, id)}/>
            </div>
        )
    }
            ...
            <div>
                <div ref="inProgressRef">
                    <div className="status-content in-progress-status">
                        {!(this.state.jobType == '1099' && this.state.paymentType == 'Hourly Rate/Fixed Fee') ?
                            <h6>This job is in progress. Here’s a look at your upcoming and completed
                                deliverables{this.isSeeker() ? '.' : ' for this project.'}</h6>
                            :
                            <h6>This job is in progress. Here’s a look at your upcoming
                                deliverable{this.isSeeker() ? '.' : ' for this project.'}</h6>
                        }
                    </div>
                    <div className="in-progress-desktop">
                        {!(this.state.jobType == '1099' && this.state.paymentType == 'Hourly Rate/Fixed Fee') ?
                            <ul className="nav nav-pills nav-justified">
                                <li className="active"><a data-toggle="pill" href="#pending_deliverables"
                                                          onClick={(e) => this.onTabChange(e, TAB_TYPE['PENDING'])}>Pending
                                    Deliverables</a></li>
                                <li><a data-toggle="pill" href="#past_deliverables"
                                       onClick={(e) => this.onTabChange(e, TAB_TYPE['PAST'])}>Past Due Deliverables</a>
                                </li>
                                <li><a data-toggle="pill" href="#completed_deliverables"
                                       onClick={(e) => this.onTabChange(e, TAB_TYPE['COMPLETED'])}>Completed
                                    Deliverables</a></li>
                            </ul>
                            :
                            <ul className="nav nav-pills nav-justified">
                    ...
                    <div className="in-progress-responsive">
                        <div id="deliverables_carousel" className="carousel slide custom-carousel" data-ride="carousel"
                             data-interval="false">
