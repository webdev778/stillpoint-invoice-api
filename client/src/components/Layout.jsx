...
class Layout extends React.Component {
  constructor(props, context) {
    ...
    this.state = {
      ...
      showHeader: false,
      currentPage: null,
      noHeaderUrlsArray : ['sign-in', 'sign-up', 'forgot-password', 'reset-password', 'verify-email'],
      headerKey: 0,
      logoImage: null,
      isTransHeader: false
    };
  ...
  componentDidMount() {
    browserHistory.listen(location => {
      this.setState({isSession: sessionManager.isSession()}, function() {
        ...
        this.setState({
          showHeader: !this.state.noHeaderUrlsArray.includes(location.pathname.split('/')[1]),
  ...
  render() {
    ...
    var childrenWithProps = React.Children.map(children, child =>
      React.cloneElement(child, { forceUpdateHeader: this.forceUpdateHeader, session: this.state.isSession }));

    return (
      <html>
        <head>
          <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" />
          <meta name="Description" content={constant['SEO_META_INFO']['DESC'][this.props.location.pathname] || ''} />

          <title>{constant['SEO_META_INFO']['TITLE'][this.props.location.pathname] || 'Legably'}</title>
        ...
        <body>
          ...
          {
            this.state.showHeader ? <LegablyHeader headerKey={this.state.headerKey} session = { this.state.isSession } homepage = { this.state.isHomePage} currentPage = {this.state.currentPage} logoImage={this.state.logoImage} isTransHeader={this.state.isTransHeader} /> : null
          }
