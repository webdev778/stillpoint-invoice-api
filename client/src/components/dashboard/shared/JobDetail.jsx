                ...
                <section className="job-details-wrapper">
                    <div className="section-head">
                        <ol className="breadcrumb">
                            ...
                            <p>
                                <Link to={headerLink}> {headerText} </Link><i className="fa fa-angle-right mr-15 ml-15"
                                                                              aria-hidden="true"></i> Job Detail
                            </p>
                        </ol>
                        {
                            (this.isFromPostedJob(fromRoute) && jobDetails.currentHighestJobStep < constant['JOB_STEPS']['IN_PROGRESS']) ?
                                <div className="d-inline">
                                    <button type="button" className="btn edit-profile-btn pull-right"
                                            onClick={() => utils.changeUrl(location.pathname + '/edit')}>
                                        <a>
                    ...
                    {this.isFromPostedJob(fromRoute) ?
                        this.state.posterJobStatus === constant['STATUS']['ACTIVE'] ?
                            <JobStepsView role='poster' jobType={jobDetails.jobType}
                                          paymentType={jobDetails.paymentType} step={jobDetails.currentHighestJobStep}
                                          stepRelatedData={jobDetails.stepData}
                                          declinedCandidateList={jobDetails.declinedCandidates} jobId={jobDetails.jobId}
                                          userId={jobDetails.userId}/>
                            :
                            null
                        :
                        jobStatus !== constant['JOB_STEPS']['APPLY'] ?
                            <JobStepsView role='seeker' jobType={jobDetails.jobType}
                                          paymentType={jobDetails.paymentType} step={jobStatus}
                                          stepRelatedData={jobDetails.stepData} jobId={jobDetails.jobId}
                                          freezeActivity={this.state.freezeActivity} userId={jobDetails.userId}/>
                            :
                            null
                    }
                                ...
                                <div className="card-content">
                                    <div>
                                        ...
                                        <div className="hourly-estimate col-xs-3 col-sm-2 p-0">
                                            ${jobDetails.rate}
                                            <div>{jobDetails.rateType ? (jobDetails.rateType).toLowerCase() : jobDetails.rateType} (Est.)</div>
                                        </div>
                                        <span className="clearfix"></span>
                                    </div>
                                    <div className="mt-0 mb-0 job-detail-list">
                                        ...
                                        <p className="text-capitalize">
                                            <i className="fa fa-calendar" aria-hidden="true"></i>est.
                                            time: {jobDetails.hours} hrs., {jobDetails.duration} {jobDetails.durationPeriod}
                                        </p>
                                        {jobDetails.city ?
                                            <p className="text-capitalize">
                                                <i className="fa fa-map-pin"
                                                   aria-hidden="true"></i> {jobDetails.city + ', ' + jobDetails.stateAbbr}
                                            </p>
                                            : ''
                                        }
                                    ...
                                    <div className="card-content-footer row">
                                        <div className="col-sm-6">
                                            <div
                                                className="pb-5">Posted:<span>{(this.state.posterJobStatus && jobDetails.posted_at) ? utils.convertUtcToEst(jobDetails.posted_at).format(constant['JOB_DATE_FORMAT']) : constant['NO_DATA_SYMBOL']}</span>
                                            </div>
                                            <div>Desired Start
                                                Date:<span>{jobDetails.estimatedStartDate ? moment(jobDetails.estimatedStartDate).format(constant['JOB_DATE_FORMAT']) : 'ASAP'}</span>
                                            </div>
                                        </div>
                            ...
                            */}
                                            ...
                                            {this.isFromPostedJob(fromRoute) ?
                                                this.state.posterJobStatus === constant['STATUS']['INACTIVE'] ?
                                                    <button type="button" className="btn mr-10 btn-unposted"
                                                            onClick={this.updatePostedJobStatus.bind(this, this.state.jobId, constant['STATUS']['ACTIVE'])}>
                                                        Un-Posted
                                                    </button>
                                                    : null
                                                :
                                                this.state.jobStatus === constant['JOB_STEPS']['APPLY'] ?
                                                    <button type="button" className={applyBtnClass}
                                                            onClick={this.updateJobStatus.bind(this, this.state.jobId, constant['JOB_STEPS']['APPLIED'])}>
                                                        Apply Now
                                                    </button>
                                                    : null
                                            }
