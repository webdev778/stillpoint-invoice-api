            ...
            if (err) {
                ...
                if (utils.isResSuccess(response)) {
                    ...
                    if (index === null) {
                        ...
                        if (that.state.stepRelatedData.length > 0 || hStep > constant['JOB_STEPS']['INTERVIEWING']) {
                            that.setState({
                                'highestStep': hStep,
                ...
                } else if (response['data']['code'] === constant['HTTP_STATUS_CODES']['UNPROCESSABLE']) {
                    helper.openInfoMessagePopup(that, 'CANDIDATE_IN_NEGOTIATING_TERM');
                } else if (utils.isResLocked(response)) {
                    let key = (that.props.role === constant['ROLE']['SEEKER']) ? 'ACCOUNT_FROZEN_SEEKER' : 'ACCOUNT_FROZEN_POSTER';
                    ...
                    highestStep === (constant['JOB_STEPS']['INTERVIEWING'] * -1) ?
                        ...
                        (
                            <div className="status-content mt-45">
                                ...
                                <p>The hiring manager for this position is interested in discussing the role with
                                    you.</p>
                                <p>Get started by reaching out via the Legably messaging system.</p>
                                {highestStep < 0 ?
                                    null
                                    :
                                    <div className="btns text-right">
                                        {
                                            highestStep > constant['JOB_STEPS']['INTERVIEWING'] ?
                                                null
                                                :
                                                <button type="button" className={withdrawBtnSeeker}
                                                        onClick={this.onWithdrawBtnClick.bind(this, jobId, (constant['JOB_STEPS']['INTERVIEWING'] * -1), null, constant['ROLE']['SEEKER'])}>
                                        ...
                                        stepRelatedData.map((item, index) => (
                                            <div key={index} className="candidate-data">
                                                <div className="pull-left pr-30">
                                                    <img src={this.getPhotoUrl(item.job_seeker_info.network.photo)}
                                                         alt="profile-img" onError={this.profileImgError}/>
                                                </div>
                                                <div className="right-panel p-0">
                                                    ...
                                                    <p className="para mt-10 mb-20">
                                                        <Truncate lines={2} ellipsis={<span>... <Link className="more"
                                                                                                      to={this.userDetailLink(item._id)}>more</Link></span>}>
                                                    ...
                                                    <div className="buttons text-right">
                                                        ...
                                                        <button type="button"
                                                                className={item.freeze_activity ? "btn btn-primary seized-btn" : "btn btn-primary"}
                                                                onClick={this.onDeclineBtnClick.bind(this, jobId, (constant['JOB_STEPS']['INTERVIEWING'] * -1), item._id, index)}>
                                                            decline
                                                        </button>
                                                        <button type="button" className="btn btn-primary"
                                                        ...
                                                        <button type="button"
                                                                className={item.freeze_activity ? "btn btn-primary seized-btn" : "btn btn-primary"}
                                                                onClick={this.onNegotiateTermsBtnClick.bind(this, jobId, constant['JOB_STEPS']['N_TERMS'], item._id, index)}>
                                                            Negotiate Terms
                                                        </button>
                                                    </div>
                                                </div>
